...
obstacles = [
    ...
    (random.uniform(FIELD_X_MIN + 7000, FIELD_X_MAX - 500), random.uniform(FIELD_Y_MIN + 5000, FIELD_Y_MAX - 500))
    for _ in range(0)
]

vertical_line = [0, 0]  # 初期化（後で更新）
...
def plan_path():
    ...
    vertical_line[0] = -1/line[0]  # 直線の傾きを保存
    vertical_line[1] = line[1]  # 直線の切片
    print(f"Vertical line equation: y = {line[0]} * x + {line[1]}")
...
if path:
    ...
    x = np.linspace(FIELD_X_MIN, FIELD_X_MAX, 100)
    print(f"y = {vertical_line[0]} * x + {vertical_line[1]}")
    y = vertical_line[0] * x + vertical_line[1]
    plt.plot(x, y, 'k--', label='Vertical Line')  # 垂直線の描画
