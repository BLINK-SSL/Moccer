...
# 障害物の位置（フィールド内に配置）
obstacles = [
    # (random.uniform(5000, FIELD_X_MAX - 500), random.uniform(3500, FIELD_Y_MAX - 500))
    (random.uniform(FIELD_X_MIN + 8000, FIELD_X_MAX - 10), random.uniform(FIELD_Y_MIN + 6000, FIELD_Y_MAX - 10))
    for _ in range(16)
]
...
def line_equation(start, end):
    ...
    b = start[1] - a * start[0]
    # b = (target_position[1] + current_position[1]) / 2 - a * (target_position[0] + current_position[0]) / 2  # 目標位置を基準に
    return a, b
...
def generate_intermediate_targets(n=5):
    ...
    vertical_lin = (-1 / line[0], line[1])
    vertical_line = (-1 / line[0], (target_position[1] + current_position[1]) / 2 - vertical_lin[0] * (target_position[0] + current_position[0]) / 2)
    for num in range(6):
        if num % 2 == 0:
            x = (current_position[0] + (target_position[0] + current_position[0])) / (num + 2)
            x += (target_position[0] + current_position[0]) / 2
        else:
            x = -x
        print(f"Generating target for num={num}, x={x}")
        
        y = vertical_line[0] * x + vertical_line[1]
        targets.append((x, y))  # 垂直な直線上の
    # x = (current_position[0] + (target_position[0] + current_position[0])) / 2
    # y = vertical_line[0] * x + vertical_line[1]
    # targets.append((x, y))  # 中間目標を直線上に追加
    # y = vertical_line[0] * -x + vertical_line[1]
    # targets.append((-x, y))  # 反対側の中間目標も追加
    # x = (current_position[0] + (target_position[0] + current_position[0])) / 3
    # y = vertical_line[0] * x + vertical_line[1]
    # targets.append((x, y))  # さらに別の中間目標を
